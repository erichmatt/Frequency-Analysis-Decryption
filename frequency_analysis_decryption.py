# -*- coding: utf-8 -*-
input_text = """We believe university-level education can be both high quality
and low cost. Using the economics of the Internet, we've connected some of the
greatest teachers to hundreds of thousands of students in almost every country
on Earth. Udacity was founded by three roboticists who believed much of the
educational value of their university classes could be offered online for very
low cost. A few weeks later, over 160,000 students in more than 190 countries
enrolled in our first class, "Introduction to Artificial Intelligence." The class
was twice profiled by the New York Times and also by other news media. Now we're
a growing team of educators and engineers, on a mission to change the future of
education."""

frequency_text = '''Artificial intelligence (AI) is the intelligence of machines and the branch of computer science that aims to create it. AI textbooks define the field as "the study and design of intelligent agents"[1] where an intelligent agent is a system that perceives its environment and takes actions that maximize its chances of success.[2] John McCarthy, who coined the term in 1956,[3] defines it as "the science and engineering of making intelligent machines."[4] Artificial intelligence takes the intelligence of humans, such as perception, natural language processing, problem solving and planning, learning and adaptation, and acting on the environment and applies them with machines, systems, and virtual objects.
...
They had failed to recognize the We believe university-level education can be both high quality
and low cost. Using the economics of the Internet, we've connected some of the
greatest teachers to hundreds of thousands of students in almost every country
on Earth. Udacity was founded by three roboticists who believed much of the
educational value of their university classes could be offered online for very
low cost. A few weeks later, over 160,000 We believe university-level education can be both high quality
and low cost. Using the economics of the Internet, we've connected some of the
greatest teachers to hundreds of thousands of students in almost every country
on Earth. Udacity was founded by three roboticists who believed much of the
educational value of their university classes could be offered online for very
...
In common usage, the term "AI" no longer seems to apply to off-the-shelf solved computing-science problems, which may have originally emerged out of years of AI research. ARDUINO 1.0
...
def pairs_update_prob(base_pairs,cypher_pairs,prob):
    for p in range(min([len(base_pairs)-2,len(cypher_pairs)-2])):
        if base_pairs[p][0][0] in prob:
            if cypher_pairs[p][0][0] in prob[base_pairs[p][0][0]]:
                prob[base_pairs[p][0][0]][cypher_pairs[p][0][0]] += .03
            if cypher_pairs[p+1][0][0] in prob[base_pairs[p][0][0]]:
                prob[base_pairs[p][0][0]][cypher_pairs[p+1][0][0]] += .02
            if cypher_pairs[p+2][0][0] in prob[base_pairs[p][0][0]]:
                prob[base_pairs[p][0][0]][cypher_pairs[p+2][0][0]] += .01
        if base_pairs[p][0][1] in prob:
            if cypher_pairs[p][0][1] in prob[base_pairs[p][0][1]]:
                prob[base_pairs[p][0][1]][cypher_pairs[p][0][1]] += .03
            if cypher_pairs[p+1][0][1] in prob[base_pairs[p][0][1]]:
                prob[base_pairs[p][0][1]][cypher_pairs[p+1][0][1]] += .02
            if cypher_pairs[p+2][0][1] in prob[base_pairs[p][0][1]]:
                prob[base_pairs[p][0][1]][cypher_pairs[p+2][0][1]] += .01
    #return prob
...
text_freq = frequency(alph,input_text)

cypher_freq = frequency(cypher_alph,cypher_text)

base_pairs = find_pairs(input_text,alph)
